...
class RestaurantsAdapter(val context: Context, val restaurants: List<YelpRestaurant>) :
    ...
    inner class ViewHolder(itemView: View): RecyclerView.ViewHolder(itemView) {
        fun bind(restaurant: YelpRestaurant) {
            ...
            itemView.rtRating.rating = restaurant.rating.toFloat()
            itemView.tvNumOfReviews.text = "${restaurant.numReviews} Reviews"
            ...
            Glide.with(context).load(restaurant.imageUrl).apply(RequestOptions().transforms(CenterCrop(), RoundedCorners(20))).into(itemView.ivRestaurant)
        }
